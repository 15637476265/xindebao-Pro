...
cv::Mat edgeCurveDetector::detector(cv::Vec4d &lineLeft, cv::Vec4d &lineRight, const cv::Mat &input, int range)
{
    ...
    dst= cv::Mat::zeros(_img.rows,_img.cols,CV_8UC1);
...
//    //cout << dstl.depth() << " " << dstl.channels() << endl;
//    dst(cv::Rect2i(0,0,_img.cols/2,_img.rows))= dstl;
//    dst(cv::Rect2i(_img.cols/2,0,_img.cols/2,_img.rows))= dstr;
//    cv::namedWindow( "i", CV_WINDOW_AUTOSIZE );
//    cv::imshow("i",dst);
//    cv::waitKey(0);
...
//    cv::imshow("d",dst);
//    cv::waitKey(0);
    if(pts.size() < dstl.rows/5)
    {
        //cout << "ransac_aglo " << endl;
        return cv::Mat::zeros(0,0,CV_8UC1);
    }
    else
    {
        fitLine(pts, lineLeft, cv::DIST_HUBER, 0, 0.01,0.01);
        double k= lineLeft[1]/lineLeft[0];
        if(k < 0 || k > 1) return cv::Mat::zeros(0,0,CV_8UC1);
    }
...
//    cv::waitKey(0);
    ...
    else
    {
        ...
        double k= lineRight[1]/lineRight[0];
        if(k > 0 || k < -1) return cv::Mat::zeros(0,0,CV_8UC1);
    }
...
cv::Mat edgeCurveDetector::detector2(cv::Vec4d &lineLeft, cv::Vec4d &lineRight, const cv::Mat &input, int range)
{
    cv::Mat timg255,r;
    cv::threshold(_img,timg255,254,255,1);
    //_img.copyTo(r,timg255);
    cv::Moments m= cv::moments(255-timg255);
    int centerx= int(m.m10/m.m00);
    cv::line(r,cv::Point(centerx,0),cv::Point(centerx,1280),0,5);
    //return r;
    cv::Mat dstl,dstr,dst;
    dst= cv::Mat::zeros(_img.rows,_img.cols,CV_8UC1);
    cv::filter2D(_img(cv::Rect2i(0,0,centerx,_img.rows)),dstl,-1,_kernel);
    cv::filter2D(_img(cv::Rect2i(centerx,0,_img.cols-centerx,_img.rows)),dstr,-1,-_kernel);
    ...
    cv::imwrite("dl.png",dst);
//    cv::namedWindow( "d", CV_WINDOW_AUTOSIZE );
//    cv::imshow("d",dst);
//    cv::waitKey(0);
    if(pts.size() < dstl.rows/3)
    {
        //cout << "ransac_aglo /3" << endl;
        if(!ranSac(pts, 55, 5, 2.8, lineLeft,1)) return cv::Mat::zeros(0,0,CV_8UC1);
    }
    else
    {
        fitLine(pts, lineLeft, cv::DIST_HUBER, 0, 0.01,0.01);
...
//    }
    ...
    for(int i= 0;i < dstr.rows;i++)
    {
        roi.y= i;
        cv::minMaxLoc(dstr(roi),&minVal,&maxVal,&minLoc,&maxLoc);
        pts.push_back(cv::Point(i,maxLoc.x+roi.x+centerx));
        dst.at<uchar>(i,maxLoc.x+roi.x+centerx)= maxVal;
        ////cout << i << ": " << minVal << endl;
    }
//    cv::namedWindow( "d", CV_WINDOW_AUTOSIZE );
//    cv::imshow("d",dst);
//    cv::waitKey(0);
    ...
    if(pts.size() < dstr.rows/3)
    {
        //cout << "ransac_aglo " << endl;
        if(!ranSac(pts, 55, 5, 2, lineRight,-1)) return cv::Mat::zeros(0,0,CV_8UC1);
    }
    else
    {
        fitLine(pts, lineRight, cv::DIST_HUBER, 0, 0.01,0.01);
...
//    if(count*1.0/dstr.rows > 0.33) return cv::Mat::zeros(0,0,CV_8UC1);
    lineRight[2]= (dr+lineRight[1]*0)/lineRight[0];
    lineRight[3]= (dr+lineRight[1]*dstl.rows)/lineRight[0];
    lineRight[0]= (lineRight[2]+0)*_resize;
    lineRight[1]= 0;
    lineRight[2]= (lineRight[3]+0)*_resize;
    lineRight[3]= _img.rows*_resize;
//    double dr= lineRight[0]*(lineRight[3]+roisl.y+mroi.y)-lineRight[1]*(lineRight[2]+roisl.x+mroi.x); //cout << lineRight << " " << dr << endl;
//    slcut0.y= (dr+lineRight[1]*(slcut0.x))/lineRight[0];
//    slcut1.y= (dr+lineRight[1]*(slcut1.x))/lineRight[0];
