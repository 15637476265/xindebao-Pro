...
int AdjustCamera::enumCamDevice()
{
...
void AdjustCamera::preConnectDevice()
{
    ...
    if ( (m_device_handle != NULL) && (!m_is_open) )
    {
...
void AdjustCamera::connectDevice(std::string _index)
{
    bool ret  = true;
    GX_STATUS status = GX_STATUS_SUCCESS;
...
void AdjustCamera::disconnectDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS AdjustCamera::OpenDevice()
{
    ...
    open_param.pszContent  = const_cast<char*>(serial.c_str());
    status = GXOpenDevice(&open_param, &m_device_handle);

    return status;
}

GX_STATUS AdjustCamera::CloseDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS AdjustCamera::InitDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS AdjustCamera::SetPixelFormat8bit()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS AdjustCamera::GetDeviceInitParam()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
bool AdjustCamera::PrepareForShowImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    bool ret = false;
...
bool  AdjustCamera::PrepareForShowColorImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    //创建QImage对象，用于显示图像
...
*/
//----------------------------------------------------------------------------------
bool  AdjustCamera::PrepareForShowMonoImg()
{
    //创建QImage对象，用于显示图像
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    m_show_image = new QImage(m_image_width, m_image_height, QImage::Format_RGB888);
...
void AdjustCamera::UnPrepareForShowImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    if(m_frame_data.pImgBuf != NULL)
    {
...
cv::Mat AdjustCamera::getImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    return  const_cast<AdjustCamera*>(this)->takePicture() ;
}

GX_FRAME_DATA AdjustCamera::getFrameDate()
{
    return m_frame_data;
}

bool AdjustCamera::isDeviceOpend()
{
    return m_is_open;
}

cv::Mat AdjustCamera::takePicture()
{
    ...
    {
        connectDevice(_cam_index);
        disconnectDevice();
    }
    return _mat;
}

void AdjustCamera::setExposure(int exposure)
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
void AdjustCamera::setGain( float gain )
{
    std::lock_guard<std::recursive_mutex> lck(_mtx) ;
    if(m_device_handle == nullptr)
        return;
...
void AdjustCamera::sendStartCapture()
{
    GX_STATUS status = GX_STATUS_SUCCESS;
...
void AdjustCamera::sendStopCapture()
{
    GX_STATUS status = GX_STATUS_SUCCESS;
    if(m_is_snap)
    {
...
void AdjustCamera::setRoi( float offsetX, float offsetY,int width,int height)
{
    std::lock_guard<std::recursive_mutex> lck(_mtx) ;
    if(m_device_handle == nullptr)
        return;
